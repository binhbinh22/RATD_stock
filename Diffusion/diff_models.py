...
class ReferenceModulatedCrossAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cond_to_k = nn.Linear(2*dim +  context_dim, inner_dim, bias = False)  # them 2 vao context dim
        # print(self.top_k)
        # print("Hello")
        # print(dim)
        # print(context_dim)
        ...
        #print((2*dim+ self.top_k*context_dim, inner_dim))
        self.ref_to_v = nn.Linear(dim +  context_dim, inner_dim, bias = False) ## ban dau khong co 2

        self.to_out = nn.Linear(inner_dim, dim)
        self.context_to_out = nn.Linear(inner_dim, context_dim)
        # self.encode_image = nn.Linear(512*top_k, 24*top_k)
    ...
    def forward(
        ...
        #print(B, C, K, L) #8, 64, 1, 120
        # print("C = ", C)
        x = self.norm(x)
        #reference = self.norm(reference)
        # print("diffusion check:")
        # print(reference.shape)
        #reference_image = self.norm(reference_image)
        #print(reference_image.shape)
        #reference_image = self.encode_image(reference_image)
        # print("Sau khi encode_image")
        # print(reference_image.shape)
        #reference = torch.cat([reference, reference_image], dim=-1)
        # print("Sau khi concat")
        # print(reference.shape)
        cond_info = self.context_norm(cond_info)
        # print("reference")
        # print(reference.shape)
        reference = repeat(reference, 'b n c -> (b f) n c', f= int(C))# (B*C, K, L)
        #print(reference.shape)
...
class diff_RATD(nn.Module):
    def __init__(self, config, inputdim=2, use_ref=True, top_k = 1):
        super().__init__()
...
class ResidualBlock(nn.Module):
    def __init__(self, side_dim, ref_size, h_size, channels, diffusion_embedding_dim, nheads, is_linear=False, top_k = 1):
        super().__init__()
        ...
        # print("top_k")
        # print(self.top_k)
        self.RMA=ReferenceModulatedCrossAttention(dim=ref_size+h_size,context_dim=ref_size*self.top_k, top_k = self.top_k)
        self.line= nn.Linear(
                ref_size*self.top_k, ref_size+h_size
            )
