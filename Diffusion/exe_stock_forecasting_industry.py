...
from utils_industry import train, evaluate
...
parser.add_argument("--csv_path", type=str, default="GOOG_filtered.csv")
parser.add_argument("--device", default="cuda:1", help="Thiết bị chạy model")
...
for top_k in top_k_list:
    for step_size in step_size_list:
        reference =  torch.load(f'{stock}_k_n_industry/{stock}_vs_industry_{top_k}_{step_size}.pt')
        ...
        print("Top_k:", top_k)
        model = RATD_Forecasting(config, args.device, args.target_dim, top_k = top_k).to(args.device)
        ...
        evaluate(model,stock,seq, pred, test_loader, nsample=args.nsample, scaler=1, mean_scaler=0, foldername=foldername, top_k = top_k, step_size = step_size, seed = seed)
